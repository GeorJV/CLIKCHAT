...
export const BusinessSettingsTab: React.FC<BusinessSettingsTabProps> = ({
  ...
  const [activeSubTab, setActiveSubTab] = useState<'identity' | 'faqs'>(initialSubTab);

  return (
    <div className="space-y-4 max-w-5xl text-slate-100 font-sans">
      {/* Encabezado y Selector de Sub-secciones */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#242323] pb-3.5">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-0.5">
            Configura la identidad, tipo de industria y preguntas frecuentes (FAQs) de tu negocio en un solo lugar.
          </p>
        </div>
        ...
        <div className="flex items-center gap-1.5 bg-[#121111] p-1 rounded-xl border border-[#262424] shrink-0 self-start sm:self-auto">
          ...
          >
            ...
            <span>Identidad & Tipo de Negocio</span>
          </button>
      ...
      {/* Contenido Dinámico de la Sub-sección */}
      {activeSubTab === 'identity' ? (
        ...
        <FaqsManagerTab
          faqs={faqs}
          onCreateFaq={onCreateFaq || (async () => false)}
          onDeleteFaq={onDeleteFaq || (async () => false)}
          onUpdateFaq={onUpdateFaq}
          onCreateBulkFaqs={onCreateBulkFaqs}
        />
      )}
    </div>
  );
