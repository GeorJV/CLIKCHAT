        ...
        {messages.map((msg, idx) => {
          ...
          return (
            <div key={msg.id} className={`flex items-start ${isAssistant ? 'justify-start' : 'justify-end'} ${isSameSender ? 'mt-0.5' : idx === 0 ? 'mt-0' : 'mt-2'}`}>
              <div className={`w-fit max-w-[85%] sm:max-w-[78%] ${isAssistant ? 'px-3 py-1.5' : 'px-2.5 py-0.5'} text-xs sm:text-sm animate-bubble-in ${isAssistant ? `${themeStyles.botBubble} origin-bottom-left` : `${themeStyles.userBubble} origin-bottom-right`}`}>
