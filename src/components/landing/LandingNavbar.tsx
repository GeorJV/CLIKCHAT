...
import { Sparkles, Menu, X, Gem } from 'lucide-react';
    ...
    <header className="sticky top-0 z-50 w-full bg-[#fbfaf8]/90 backdrop-blur-xl border-b border-[#e9e5dc] transition-all">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-18 flex items-center justify-between">
        ...
        <div className="hidden sm:flex items-center space-x-3">
          <button
            ...
            className="px-4 py-2.5 text-xs font-semibold text-[#2c2824] hover:text-black transition rounded-xl border border-[#dcd6cb] hover:bg-[#f2eee5] cursor-pointer shadow-[0_1px_2px_rgba(0,0,0,0.02)]"
          >
            Acceso a Tienda
          </button>
          <button
            onClick={onOpenDemo}
            className="px-5 py-2.5 text-xs font-bold text-[#fbfaf8] bg-[#181716] hover:bg-[#2c2925] border border-[#2e2a25] transition rounded-xl shadow-md flex items-center gap-2 cursor-pointer active:scale-95"
          >
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-[#fbfaf8] border-b border-[#e9e5dc] px-5 py-5 space-y-3.5 shadow-xl animate-fadeIn">
          ...
          <div className="pt-3 flex flex-col gap-2.5 border-t border-[#e2ddd4]">
            <button onClick={onGoToDashboard} className="w-full py-2.5 text-xs font-semibold text-[#2c2824] border border-[#dcd6cb] rounded-xl text-center bg-white">Acceso a Tienda</button>
            <button onClick={onOpenDemo} className="w-full py-2.5 text-xs font-bold text-white bg-[#181716] rounded-xl text-center shadow-md">Probar Demostración</button>
          </div>
        </div>
      )}
    </header>
