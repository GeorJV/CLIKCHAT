  ...
  return (
    <section className="flex flex-col h-full bg-[#131212] overflow-hidden relative min-h-0 select-none">
      <div className="flex-1 flex flex-col px-3 pt-0 pb-3 overflow-hidden min-h-0">
        {/* Showcase Image to the very top */}
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center gap-2 mt-1 text-[11px] text-zinc-300">
                  <span className="flex items-center gap-1 text-amber-400 font-bold">
                    ...
                    {service.rating || 4.9} (Reseñas verificadas)
                  </span>
                  <span>•</span>
                  <span className="flex items-center gap-1 text-zinc-300">
                    <Clock className="w-3.5 h-3.5 text-emerald-400" />
                    {service.duration || '45 minutos'}
                  </span>
                </div>
              </div>
        ...
        {/* Action Inquiry Buttons - Clean, transparent without heavy background or borders */}
        <div className="grid grid-cols-2 gap-2 mt-2.5 shrink-0">
          <button
            type="button"
            onClick={onOpenIncludes}
            className="flex items-center justify-center gap-2 py-2 px-2.5 rounded-xl text-xs font-semibold transition text-zinc-300 hover:text-white hover:bg-white/[0.04] cursor-pointer active:scale-98"
          >
            <Sparkles className="w-3.5 h-3.5 text-emerald-400" />
            <span className="truncate">¿Qué incluye la sesión?</span>
          </button>
          ...
          >
            ...
            <span className="truncate">Requisitos y Garantía</span>
          </button>
        </div>

        {/* Primary Booking Button: No direct numeric price to encourage consultation */}
